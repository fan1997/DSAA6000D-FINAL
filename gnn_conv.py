...
class TCGNNFunction_SAG(torch.autograd.Function):
    ...
    def forward(ctx, X, row_pointers, column_index, \
        ...
        print("mean:", torch.mean(X_out))
        print("max:", torch.max(X_out))
        print("min:", torch.min(X_out))
        return X_out
...
class TCGNNFunction_SAG_balance(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, row_pointers, column_index, \
                blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count):

        ctx.backward_ = (row_pointers, column_index, \
                                blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)

        # Basic Scatter and Gather
        X_out = TCGNN.balance_forward(X, row_pointers, column_index, \
                                blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)[0]
        print(torch.mean(X_out))
        return X_out

    @staticmethod
    def backward(ctx, d_output):
        row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow,TCblock_rowid, TCblock_colid, tc_count = ctx.backward_
        # SAG backward.
        d_input = TCGNN.balance_forward(d_output, row_pointers, column_index, \
                                blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)[0]
        return d_input, None, None, None, None, None, None, None, None

class TCGNNFunction_SAG_balance_v1(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes):
        ctx.backward_ = (TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes)
        # Basic Scatter and Gather
        X_out = TCGNN.balance_forward_v1(X, TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes)[0]
        print("mean:", torch.mean(X_out))
        print("max:", torch.max(X_out))
        print("min:", torch.min(X_out))
        return X_out
    @staticmethod
    def backward(ctx, d_output):
        TCblock_rowid, TCblock_colid, TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes = ctx.backward_
        # SAG backward.
        d_input = TCGNN.balance_forward_v1(d_output, TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes)[0]
        return d_input, None, None, None, None, None, None, None, None
...
class TCGNNFunction_Balance(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, weights, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count):
        ctx.backward_ = (X, weights, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)
        # GEMM node update
        X_prime = torch.mm(X, weights)
        # SpMM: Neighbor AggreAGNNion.
        X_prime = TCGNN.balance_forward(X_prime, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)[0]
        return X_prime

    @staticmethod
    def backward(ctx, d_output):
        X, weights, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count = ctx.backward_
        # SPMM backward propaAGNNion.
        d_input_prime = TCGNN.balance_forward(d_output, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)[0]
        # GEMM backward propaAGNNion.
        d_input = torch.mm(d_input_prime, weights.transpose(0,1))
        d_weights = torch.mm(X.transpose(0,1), d_input_prime)
...
class SAG_balance(torch.nn.Module):
    def __init__(self, row_pointers, column_index, \
                    blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count):
        super(SAG_balance, self).__init__()
        ...
        self.edgeToColumn = edgeToColumn
        self.edgeToRow = edgeToRow
        self.TCblock_rowid = TCblock_rowid
        self.TCblock_colid = TCblock_colid
    ...
    def profile(self, X, num_rounds=200):
        ...
        for _ in tqdm(range(num_rounds)):
            TCGNNFunction_SAG_balance.apply(X, self.row_pointers, self.column_index, \
                                        self.blockPartition, self.edgeToColumn, self.edgeToRow, self.TCblock_rowid, self.TCblock_colid, self.tc_count)
        torch.cuda.synchronize()
        dur = time.perf_counter() - start
        print("=> SAG balance profiling avg (ms): {:.3f}".format(dur*1e3/num_rounds))
        print()

class SAG_balance_v1(torch.nn.Module):
    def __init__(self, TCblock_rowid, TCblock_colid, TCblocktile_rowid, TCblocktile_colid, TCblock_offset, sparse_AToX_idx, num_nodes):
        super(SAG_balance_v1, self).__init__()
        self.TCblock_rowid = TCblock_rowid
        self.TCblock_colid = TCblock_colid
        self.TCblocktile_rowid = TCblocktile_rowid
        self.TCblocktile_colid = TCblocktile_colid
        self.TCblock_offset = TCblock_offset
        self.sparse_AToX_idx = sparse_AToX_idx
        self.num_nodes = num_nodes
    def profile(self, X, num_rounds=200):
        torch.cuda.synchronize()
        start = time.perf_counter()
        for _ in tqdm(range(num_rounds)):
            out = TCGNNFunction_SAG_balance_v1.apply(X, self.TCblock_rowid, self.TCblock_colid, self.TCblocktile_rowid, self.TCblocktile_colid, self.TCblock_offset, self.sparse_AToX_idx, self.num_nodes)
        torch.cuda.synchronize()
        dur = time.perf_counter() - start
        print("=> SAG balance profiling avg (ms): {:.3f}".format(dur*1e3/num_rounds))
        print()
...
class GCNConv_Balance(torch.nn.Module):
    def __init__(self, input_dim, output_dim):
        super(GCNConv_Balance, self).__init__()
        self.weights = torch.nn.Parameter(torch.randn(input_dim, output_dim))
    ...
    def forward(self, X, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count):
        '''
        @param:
        X:  the input tensor of the graph node embedding, shape: [n_nodes, n_dim].
        A:  the CSR node pointer of the graph, shape: [node, 1].
        ...
        partitioin: for the graph with the part-based optimziation.
        '''
        return TCGNNFunction_Balance.apply(X, self.weights, row_pointers, column_index, blockPartition, edgeToColumn, edgeToRow, TCblock_rowid, TCblock_colid, tc_count)
